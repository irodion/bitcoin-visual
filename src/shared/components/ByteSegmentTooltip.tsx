...
export function ByteSegmentTooltip({
  ...
  return (
    <span className={`group/seg relative inline cursor-help ${color}`}>
      <span className="rounded-sm transition-colors group-hover/seg:bg-white/5">{children}</span>
      <span className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-3 hidden w-max max-w-xs -translate-x-1/2 rounded-card border border-border-strong bg-surface-raised px-3 py-2 text-left shadow-lg group-hover/seg:block">
